...
public class EntityThaumicSlime extends EntityMob implements IMob, ITaintedMob {
   ...
   public void setThrowableHeading(double par1, double par3, double par5, float par7, float par8) {
      ...
      this.prevRotationPitch = this.rotationPitch = (float)(Math.atan2(par3, (double)var10) * (double)180.0F / Math.PI);
   }
   ...
   public void setSlimeSize(int par1) {
      ...
      float ss = (float)Math.sqrt((double)par1);
      this.setSize(0.25F * ss + 0.25F, 0.25F * ss + 0.25F);
      this.setPosition(this.posX, this.posY, this.posZ);
      this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue((double)par1);
   ...
   public void onUpdate() {
      ...
      super.onUpdate();
      int i = (int)Math.sqrt((double)this.getSlimeSize());
      if (this.launched > 0) {
         --this.launched;
   ...
   protected EntityThaumicSlime getClosestMergableSlime() {
      ...
      List ents = this.worldObj.getEntitiesWithinAABB(EntityThaumicSlime.class, AxisAlignedBB.getBoundingBox(this.posX, this.posY, this.posZ, this.posX, this.posY, this.posZ).expand((double)16.0F, (double)8.0F, (double)16.0F));
   ...
   protected void updateEntityActionState() {
      this.despawnEntity();
      EntityPlayer entityplayer = this.worldObj.getClosestVulnerablePlayerToEntity(this, (double)16.0F);
      ...
      if (this.onGround && this.slimeJumpDelay-- <= 0) {
         ...
         this.moveForward = (float)(Math.sqrt((double) this.getSlimeSize()));
      } else {
   ...
   public void onCollideWithPlayer(EntityPlayer par1EntityPlayer) {
      if (this.canDamagePlayer()) {
         int i = (int)Math.max((double)1.0F, Math.sqrt((double)this.getSlimeSize()));
