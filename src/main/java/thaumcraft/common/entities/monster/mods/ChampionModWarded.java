...
public class ChampionModWarded implements IChampionModifierEffect {
   ...
   public void showFX(EntityLivingBase boss) {
      if (!boss.worldObj.rand.nextBoolean()) {
         ...
         Thaumcraft.proxy.drawGenericParticles(boss.worldObj, boss.boundingBox.minX + (double)w, boss.boundingBox.minY + (double)h, boss.boundingBox.minZ + (double)d, (double)0.0F, (double)0.0F, (double)0.0F, 0.5F + boss.worldObj.rand.nextFloat() * 0.1F, 0.5F + boss.worldObj.rand.nextFloat() * 0.1F, 0.5F + boss.worldObj.rand.nextFloat() * 0.1F, 0.6F, true, 21, 4, 1, 4 + boss.worldObj.rand.nextInt(4), 0, 0.8F + boss.worldObj.rand.nextFloat() * 0.3F);
      }
   }
}
