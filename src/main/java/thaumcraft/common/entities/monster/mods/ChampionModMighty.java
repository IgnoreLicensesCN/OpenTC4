...
public class ChampionModMighty implements IChampionModifierEffect {
   ...
   public void showFX(EntityLivingBase boss) {
      if (!(boss.worldObj.rand.nextFloat() > 0.3F)) {
         ...
         Thaumcraft.proxy.drawGenericParticles(boss.worldObj, boss.boundingBox.minX + (double)w, boss.boundingBox.minY + (double)h, boss.boundingBox.minZ + (double)d, (double)0.0F, (double)0.0F, (double)0.0F, 0.8F + boss.worldObj.rand.nextFloat() * 0.2F, 0.8F + boss.worldObj.rand.nextFloat() * 0.2F, 0.8F + boss.worldObj.rand.nextFloat() * 0.2F, 0.7F, false, p, 3, 1, 4 + boss.worldObj.rand.nextInt(3), 0, 1.0F + boss.worldObj.rand.nextFloat() * 0.3F);
      }
   }
}
