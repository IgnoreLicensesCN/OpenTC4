...
public class PacketFXBlockZap implements IMessage, IMessageHandler<PacketFXBlockZap,IMessage> {
   ...
   public IMessage onMessage(PacketFXBlockZap message, MessageContext ctx) {
      ...
      Thaumcraft.proxy.getClientWorld().playSound((double)message.x, (double)message.y, (double)message.z, "thaumcraft:zap", 0.1F, 1.0F + Thaumcraft.proxy.getClientWorld().rand.nextFloat() * 0.2F, false);
      return null;
   }
}
